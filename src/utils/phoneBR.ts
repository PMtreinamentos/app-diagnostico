...
export function validatePhoneBR(phone: string): { valid: boolean; error?: string } {
  ...
  if (n.length < 11) return { valid: false, error: 'Digite o número completo com DDD.' };
  if (n.length > 11) return { valid: false, error: 'Número inválido.' };
  const ddd = n.slice(0, 2);
  if (!DDD_MAP[ddd]) return { valid: false, error: `DDD ${ddd} não é válido no Brasil.` };
  if (n[2] !== '9') return { valid: false, error: 'Número de celular deve começar com 9 após o DDD.' };
  return { valid: true };
}

export function getLocationFromPhone(phone: string): DddInfo | null {
